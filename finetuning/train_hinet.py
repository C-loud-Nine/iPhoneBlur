"""Fine-tune HINet - 50 epochs"""
import sys,os,torch,torch.nn as nn,torch.optim as optim
from pathlib import Path
from tqdm import tqdm

sys.path.insert(0,str(Path(__file__).parent.parent))
from models.hinet import build_hinet
from dataset.dataloader import get_dataloader

def train(epochs=50,batch_size=2,lr=1e-4,data_dir='./dataset/data/iphoneblur',ckpt_dir='./checkpoints/hinet',pretrained=None):
    ...
    model=build_hinet(device)
    print(f"HINet: {sum(p.numel() for p in model.parameters())/1e6:.1f}M params")
    ...
    for epoch in range(1,epochs+1):
        ...
        if epoch%10==0:
            torch.save({'epoch':epoch,'model_state_dict':model.state_dict(),'loss':avg},f"{ckpt_dir}/hinet_epoch_{epoch}.pth")
        if avg<best_loss:
            best_loss=avg
            torch.save({'model_state_dict':model.state_dict()},f"{ckpt_dir}/hinet_best.pth")
    torch.save({'model_state_dict':model.state_dict()},f"{ckpt_dir}/hinet_final.pth")
    print(f"✅ Done!")
...
if __name__=='__main__':
    ...
    p.add_argument('--checkpoint_dir',default='./checkpoints/hinet')
