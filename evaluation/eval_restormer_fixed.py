"""
Evaluate Restormer on iPhoneBlur - FIXED METRIC ORDER
...
def check_packages():
    ...
    print("="*70)
    packages = {'torch':'torch', 'cv2':'opencv-python', 'numpy':'numpy', 'pandas':'pandas', 'tqdm':'tqdm', 'skimage':'scikit-image', 'huggingface_hub':'huggingface_hub', 'basicsr':'basicsr'}
    for import_name, package_name in packages.items():
        try: __import__(import_name)
        except ImportError:
            try: subprocess.check_call([sys.executable, "-m", "pip", "install", package_name, "-q"]); print(f"✅ {package_name}")
            except: print(f"⚠️  {package_name} failed")
...
def find_dataset():
    for path in [Path('./dataset/data/iphoneblur'), Path('../dataset/data/iphoneblur'), Path('./data/iphoneblur'), Path('./dataset/data/iPhoneBlur')]:
        if path.exists() and (path / 'test').exists(): return path
    return None
...
def download_weights():
    ...
    try:
        weights_path = hf_hub_download(repo_id="Shafi99/iPhoneBlur_Finetune_Models", filename='restormer_final_weights.pth', force_download=False)
        import os
        size_mb = os.path.getsize(weights_path) / (1024 * 1024)
        print(f"✅ Downloaded ({size_mb:.1f} MB)\n   Location: {weights_path}\n")
        return weights_path
...
def main():
    ...
    print("="*70); print("🔍 LOCATING DATASET"); print("="*70)
    data_dir = find_dataset() if args.data_dir is None else Path(args.data_dir)
    if data_dir is None: print("❌ Dataset not found!"); sys.exit(1)
    ...
    if not blur_files: print("❌ No images!"); sys.exit(1)
    ...
    if has_metadata:
        ...
        if 'difficulty' in metadata.columns:
            for diff in ['Easy', 'Medium', 'Hard']:
                ...
                if count > 0: print(f"   {diff}: {count}")
    else:
        print("⚠️  No metadata file")
        metadata = None
    
    print(f"\n{'='*70}"); print("⚙️  DEVICE SETUP"); print("="*70)
    ...
    if device == 'cuda' and not torch.cuda.is_available():
        ...
        device = 'cpu'
    if device == 'cuda': print(f"\n✅ Using CUDA: {torch.cuda.get_device_name(0)}")
    else: print("✅ Using CPU")
    
    print(f"\n{'='*70}"); print("🔧 LOADING RESTORMER"); print("="*70)
    ...
    print(f"\n{'='*70}"); print(f"🎯 EVALUATING {len(blur_files)} IMAGES"); print("="*70); print()
    ...
    with torch.no_grad():
        ...
        for blur_path in pbar:
            ...
            if not sharp_path.exists(): continue
            blur_img = cv2.imread(str(blur_path))
            sharp_img = cv2.imread(str(sharp_path))
            if blur_img is None or sharp_img is None: continue
            ...
            if pad_h or pad_w: output_np = output_np[:h, :w, :]
            output_np = np.clip(output_np, 0, 1)
            
            # FIXED: correct order (prediction first, ground truth second)
            psnr_val = psnr_metric(output_np, sharp_img, data_range=1.0)
            ssim_val = ssim_metric(output_np, sharp_img, data_range=1.0, channel_axis=2, win_size=11)
            
            running_psnr += psnr_val
            running_ssim += ssim_val
            count += 1
            pbar.set_postfix({'PSNR': f"{running_psnr/count:.2f}", 'SSIM': f"{running_ssim/count:.4f}"})
            results.append({'img_id': blur_path.stem, 'psnr': psnr_val, 'ssim': ssim_val})
            del blur_t, output, output_np
            if device == 'cuda': torch.cuda.empty_cache()
    
    if not results: print("❌ No valid results!"); sys.exit(1)
